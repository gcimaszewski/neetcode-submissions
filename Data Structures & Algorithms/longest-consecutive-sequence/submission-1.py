    ...
    def longestConsecutive(self, nums: List[int]) -> int:
        ...
        for num in seen:
            if num - 1 not in seen:
                set_size = 1
                next_elem  = num + 1
                # this is still O(n) because each consecutive element is only discovered once
                # an element is a start of a set, OR it is a potential elemtn of a set
                # each consecutive sequence is walked only once
                while next_elem in seen:
                    next_elem += 1
                    set_size += 1
