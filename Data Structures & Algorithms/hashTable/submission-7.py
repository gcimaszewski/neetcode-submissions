...
class HashTable:
    ...
    def _insertNoResize(self, key, value, table, m):
        ...
        ll = table[h]
        if ll is None:
            table[h] = LinkedListNode(key=key, val=value)
            return True
        ...
        while ll is not None:
            if ll.key == key:
                ...
                return False
            ll = ll.next_node
        table[h] = LinkedListNode(key=key,val=value, next_=table[h])
        return True

    def insert(self, key: int, value: int) -> None:
        if (self.size+1)/self.capacity >= 0.5:
            self.resize()
        inserted = self._insertNoResize(key, value, self.bins, self.capacity)
        if inserted:
            self.size += 1
