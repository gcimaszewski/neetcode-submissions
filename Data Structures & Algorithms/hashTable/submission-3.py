...
class HashTable:
    ...
    def __init__(self, capacity: int):
        self.bins = [None for _ in range(capacity)]
    ...
    def insertNoCapacityCheck(self, key, value, table, m):
        hashed_key = self.hash(key, m)
        ll = table[hashed_key]
        if ll is None:
    ...
    def insert(self, key: int, value: int) -> None:
        added_count = self.insertNoCapacityCheck(key, value, self.bins, self.capacity)
        self.size += added_count
        if self.getLoadFactor() >= 0.5:
            self.resize()

    def get(self, key: int) -> int:
        h = self.hash(key, self.capacity)
        ll = self.bins[h]
        while ll is not None:
            if ll.key == key:
                return ll.val
    ...
    def resize(self) -> None:
        ...
        new_bins = [None for _ in range(new_capacity)]
        for ll in self.bins:
            while ll is not None:  # no need to remap empty bins
                self.insertNoCapacityCheck(ll.key, ll.val, new_bins, new_capacity)
                ll = ll.next_node
        self.bins = new_bins
